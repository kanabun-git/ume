...
- **地区の自動登録**: ジャンル欄が「業種/地区」の形式(例: 「デリヘル/錦糸町」)の場合、末尾の地区部分(「錦糸町」)を自動的に判定し、まだ登録されていない地区であれば新規登録した上で紐付けます。地区へ振り分けた後、ジャンル欄は業種部分(「デリヘル」)だけに整理され、一覧の「ジャンル」列に地名が重複して表示されることはありません。一覧の「地区」列には都道府県と合わせて「東京ー錦糸町」のように表示されます。登録される都道府県は初期値として「東京」が入りますが、[地区管理](左メニュー「地区管理」)画面で修正できます(現状の掲載サイトが東京都内中心のため)。
  ...
  - 過去に「東京」で誤登録されてしまった地区(船橋→千葉、大宮・さいたま・浦和→埼玉、高崎→群馬 など25地区)は`bin/rails shop_prospects:fix_district_prefectures`で一括修正できます(該当する地区名の都道府県だけを直し、それ以外は一切変更しません。既に正しい場合は何もしないため、再実行しても安全です)。
