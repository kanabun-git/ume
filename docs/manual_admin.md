...
- **一括削除**: 「営業先候補を一括削除する」ボタンで、一覧の内容をまとめて削除できます。進捗ステータスで絞り込んだ状態でボタンを押すと、その絞り込み結果だけが削除されます(ボタンの表示も「この絞り込み結果を一括削除する」に変わります)。押すと確認メッセージが表示され、OKした場合のみ削除が実行されます。CSV一括登録をやり直したい場合などに使用してください。**削除すると元に戻せません。**
