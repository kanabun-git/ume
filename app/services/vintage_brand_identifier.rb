...
class VintageBrandIdentifier
  ...
  SYSTEM_PROMPT = <<~PROMPT.freeze
    ...
    対象(target_gender)と想定年齢層(target_age)の出し方:
    - target_genderは「レディース」「メンズ」「ユニセックス」「キッズ」の
      いずれかで、そのブランド・ラインが対象としている区分を答えること
      (サイズ体系・カッティング・売り場の区分から判断する)
    - これらは「そのブランド・ラインが購買層として想定している対象」であって、
      その品を着ていた人物の性別や年齢ではない。着用者個人については一切推測しないこと
...

    {
      ...
      "target_gender": "レディース | メンズ | ユニセックス | キッズ のいずれか",
