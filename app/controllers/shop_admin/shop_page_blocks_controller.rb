module ShopAdmin
  class ShopPageBlocksController < BaseController
    ...
    def index
      @blocks = policy_scope(::ShopPageBlock).where(shop: current_shop)
    end

    def new
      @block = current_shop.shop_page_blocks.build(background_opacity: 1.0)
      authorize @block
    end

    def create
      @block = current_shop.shop_page_blocks.build(block_params)
      @block.position = (current_shop.shop_page_blocks.maximum(:position) || -1) + 1
      authorize @block

      if @block.save
        redirect_to shop_admin_shop_page_blocks_path, notice: "ブロックを追加しました。"
      else
    ...
    def destroy
      @block.destroy
      redirect_to shop_admin_shop_page_blocks_path, notice: "ブロックを削除しました。"
    end
    ...
    def set_block
      @block = current_shop.shop_page_blocks.find(params[:id])
      authorize @block
    end
