...
- **新規登録・編集・削除**: 店舗名・電話番号・メールアドレス・掲載元サイト名・掲載URL・ステータス・メモを管理します。
- **CSV一括登録**: 「テンプレートをダウンロード」で入力用のCSV雛形を取得し、必要事項を入力したファイルを「CSVから登録」でアップロードすると、まとめて登録できます。形式に合わない行はエラーとしてスキップされ、件数が結果画面に表示されます。
...
## 18. メールアドレス管理(左メニュー「メールアドレス管理」)

運営している各サイト(ドメイン)のメールアドレスを、この画面だけで追加・削除・送信テストできます。

### サイト(ドメイン)を登録する

画面いちばん下の「サイト(ドメイン)を追加」に、管理画面での表示名(例: サイト本体)とドメイン(例: `fuzoku-zero.com`)を入力して追加します。メールアドレスは、ここで登録したサイトごとにぶら下げて管理します。サイトは何件でも登録できます。
...
- **パスワード**: メールソフト(Thunderbird等)で受信するときに使うパスワード。10文字以上。

を入力して追加します。

### メールソフトの設定・パスワードを確認する

一覧の各アドレスの下にある「**メールソフトの設定とパスワードを表示**」を開くと、Thunderbirdやスマートフォンのメールアプリに入力する内容が一式表示されます。
...
一覧の各アドレスの行にある「送信テスト」に、自分が受け取れるメールアドレス(普段使っているGmail等)を入力して押すと、そのアドレスからテストメールが1通送信されます。結果は同じ行に「送信成功 / 送信失敗」として残るので、あとから見返せます。
...
追加・削除の内容は、通常はそのままメールサーバーに反映され、一覧に「反映済」と表示されます。「未反映」のままの場合は、画面上部の「メールサーバーへ再反映する」を押してください。それでも解消しない場合や、上部に黄色い注意書きが出ている場合は、サーバー側の設定が済んでいないため開発担当者に連絡してください。
