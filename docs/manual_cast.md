...
自分専用の来店チェックイン用QRコードを確認・印刷できます。お客様がこのQRコードをスマートフォンで読み込むと、その店舗の会員証(店舗会員証)に来店1回として自動記録され、担当キャストとして自分の名前も自動で設定されます(お客様が未ログインの場合はログイン/会員登録の後、自動的に読み込み直後の画面に戻ります)。**同じお客様が同じ日に何度も読み込んでも、記録は1日1回までです。**
