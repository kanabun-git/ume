...
- **地区の自動登録**: ジャンル欄が「業種/地区」の形式(例: 「デリヘル/錦糸町」)の場合、末尾の地区部分(「錦糸町」)を自動的に判定し、まだ登録されていない地区であれば新規登録した上で紐付けます。一覧の「地区」列には都道府県と合わせて「東京ー錦糸町」のように表示されます。登録される都道府県は初期値として「東京」が入りますが、[地区管理](左メニュー「地区管理」)画面で修正できます(現状の掲載サイトが東京都内中心のため)。
- **地区での絞り込み・一覧の分割表示**: 進捗ステータスと同様に、地区でも一覧を絞り込めます。件数が多い場合は画面に収まりきらないため、50件ごとにページ分割して表示しています(一覧上部・下部のページ送りリンクで移動できます)。
- **CSVダウンロード**: 「この一覧をCSVダウンロード」で、現在の絞り込み結果(ステータス・地区)をCSVで書き出せます。店舗名・ジャンル・地区・電話番号・メールアドレス・掲載サイト名・URL・ステータス・メール送信日時・リンククリック日時・メモを含みます(登録用CSVより列が多く、そのまま再登録用にはお使いいただけません)。
- **一括削除**: 「営業先候補を一括削除する」ボタンで、一覧の内容をまとめて削除できます。進捗ステータス・地区で絞り込んだ状態でボタンを押すと、その絞り込み結果だけが削除されます(ボタンの表示も「この絞り込み結果を一括削除する」に変わります)。押すと確認メッセージが表示され、OKした場合のみ削除が実行されます。CSV一括登録をやり直したい場合などに使用してください。**削除すると元に戻せません。**
- **営業メールの一斉送信**: 一覧の左端のチェックボックスで送信先を選び(見出し行のチェックボックスで全選択/解除)、「選択した営業先へ一斉送信する」で掲載案内メールを一斉送信できます。メールアドレス未登録の候補はチェックボックスが選択不可になっており、自動的にスキップされます。
  ...
  - **送信すると、進捗ステータスが「未アプローチ」の候補は自動的に「アプローチ済み」に変わります。**「商談中」以降に進んでいる候補へ再送信しても、ステータスは巻き戻りません。
