...
- **地区・都道府県での絞り込み・一覧の分割表示**: 進捗ステータスと同様に、都道府県単位(例: 「東京」)または地区単位(例: 「東京ー錦糸町」)で一覧を絞り込めます。件数が多い場合は画面に収まりきらないため、50件ごとにページ分割して表示しています(一覧上部・下部のページ送りリンクで移動できます)。
- **営業メール送信状況での絞り込み**: 「送信済み」「未送信」で一覧を絞り込めます。一覧のチェックボックスは表示されているページの分しか選択できないため(左上のチェックは「このページ全部」を選ぶもので、全ページ・全件を選ぶものではありません)、まず「未送信」で絞り込んでから一斉送信すると、まだ送っていない候補だけを確実に選んで送信できます。
...
- **営業メールの一斉送信**: 一覧の左端のチェックボックスで送信先を選び(見出し行のチェックボックスで、表示されているページ内の全選択/解除。他のページの候補は選択されません)、「選択した営業先へ一斉送信する」で掲載案内メールを一斉送信できます。メールアドレス未登録の候補はチェックボックスが選択不可になっており、自動的にスキップされます。選択した中に送信に失敗した候補があっても、そこで処理が止まることはなく、残りの候補には引き続き送信されます(失敗した件数は完了メッセージに表示されます)。
