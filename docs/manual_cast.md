...
**周囲へのバレ防止について**: 運営環境によっては、このマイページ(「スタッフポータル」)がサイト本体とは別の、業種を連想させない専用URLで公開されている場合があります。その場合、画面のデザイン・タイトル・アイコンもサイト本体とは異なる中立的なもの(紫〜水色のポップな配色)になります。ログイン用のURLがサイト本体のものと異なる場合は、案内された方のURLをブックマークしてご利用ください。
...
- **予約投稿**: ステータスを「公開」にしたまま「公開日時」に未来の日時を指定すると、その日時になるまで店舗ページ・キャストページ・トップページなどの公開画面には表示されません(一覧画面には「予約中」バッジが表示されます)。公開日時を空欄のまま保存すると、その場ですぐに公開されます。
...
## 6. 来店チェックインQR(左メニュー「来店チェックインQR」)

![来店チェックインQR画面](images/cast/06_check_in_qr.png)

自分専用の来店チェックイン用QRコードを確認・印刷できます。お客様がこのQRコードをスマートフォンで読み込むと、その店舗の会員証(店舗会員証)に来店1回として自動記録され、担当キャストとして自分の名前も自動で設定されます(お客様が未ログインの場合はログイン/会員登録の後、自動的に読み込み直後の画面に戻ります)。**2重にポイントが付与されるのを防ぐため、直前の記録(店舗内のどのQRコードでも)から60分以内は次の記録ができません。**

- 画面に表示されたQRコードをそのままお客様に見せて読み込んでもらうことができます。
- 「名刺PDFをダウンロード」ボタンから、自分専用のQRコードを名刺サイズ(91×55mm)のPDFとしてダウンロード・印刷できます。
- 指名区分(本指名/ネット指名/フリー)や来店時間などの詳細は自動では入力されません。店舗管理者が「店舗会員証管理」の利用履歴からあとで入力します。
