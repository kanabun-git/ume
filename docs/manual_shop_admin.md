...
**「店舗フォトギャラリー」「新人・体験入店」「店長おすすめ」「写メ日記」「週間出勤」「口コミランキング」「クーポン」の各ブロックには、この画面自体に写真・キャストなどをアップロードする欄はありません。** それぞれ「店舗情報編集」の店舗写真、「在籍キャスト管理」の体験入店/店長おすすめフラグ、キャストの写メ日記投稿、「出勤予定一括登録」の出勤予定、口コミ件数、「クーポン管理」の登録内容を、そのまま自動的に表示する仕組みです。この画面ではブロックの表示位置・表示/非表示・見た目(背景色など)だけを管理します。編集画面にも同じ案内が表示されます。
