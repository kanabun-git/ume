...
- **会員証イメージ**: 会員登録を済ませた個人会員のマイページに表示される、会員証のデザイン画像。未設定の場合は会員証セクション自体が表示されません。
...
- ランクごとに「カード画像」(会員証のベースデザイン)を登録できます。会員マイページの会員証は、その会員の現在のランクに設定されたカード画像を優先して表示し、ランクに画像が未設定の場合はサイト設定の会員証イメージ(共通デザイン)にフォールバックします。ランクをまたぐと会員証の見た目が自動的に切り替わります。
