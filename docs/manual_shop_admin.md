...
- **来店チェックインQRコード(女の子ごと)**: 女の子(キャスト)のスタッフポータル左メニュー「来店チェックインQR」に、そのキャスト専用のQRコードが表示されます。お客様がスマートフォンでこのQRコードを読み込む(会員未ログインの場合はログイン/会員登録を挟んで自動的に読み込み直後の画面に戻ります)と、来店1回として自動記録され、担当キャストにもそのキャストの名前が自動で設定されます。**同じ会員が同じ日に複数回読み込んでも、記録は1日1回までです。**
  - スタッフポータルの画面上でこの画面をお客様に見せて読み込んでもらうほか、「名刺PDFをダウンロード」から名刺サイズ(91×55mm)のQRコードカードをPDFで出力し、印刷して店頭やお客様への手渡し用にお使いいただけます。
  - 「店舗会員証管理」画面の一番上にある「在籍キャストの来店チェックインQRカードを一括ダウンロード(印刷用PDF)」から、在籍中の全キャスト分のQRコードカードを1つのPDFにまとめて出力することもできます。A4用紙1枚に10枚(2列×5段・91×55mm)ずつ並べており、市販の名刺用紙(エーワン マルチカードなど、A4・10面・91×55mmのもの)にそのまま印刷できます。
