...
- **店舗運営状況を見る(閲覧専用)**: 店舗詳細画面の下部に、その店舗管理者が普段使っている各機能を運営者側から横断的に確認できるリンクがあります。いずれも閲覧専用で、編集・削除は行えません(内容の変更が必要な場合は店舗管理者に依頼するか、上記の代理編集機能を使ってください)。
  - **在籍キャスト**: キャスト一覧と、個別の日記・出勤予定
  - **クーポン**: 登録内容と利用実績(ネット予約/手動記録の内訳)
  - **出勤予定**: 全キャストの出勤予定一覧
  - **写メ日記**: 全キャストの日記一覧と本文
  - **プレゼント企画**: 企画内容・応募者一覧・当落状況(下記の通り、こちらは運営者側から登録・編集・削除・抽選・当落メール送信まで行えます)
  - **会員ランク・特典**: 店舗独自の会員ランクと特典設定
  - **店舗会員証**: 会員ごとのランク・来店回数・ポイント残高・利用履歴・ポイント履歴・付与済み特典・事故歴/注意点(店舗内共有専用のメモも運営者は確認できます)
- **プレゼント企画は運営者側からも管理できます**: 「プレゼント企画」画面から、店舗管理者と同じ操作(新規登録・編集・削除・抽選する・当落メールを送信する)を運営者が代わりに行えます。運用ルールは店舗管理者マニュアルの「10. プレゼント企画管理」を参照してください(抽選は一度実行すると取り消せません)。
...
直近30日間の店舗詳細ページの閲覧数を確認できます。

- 全店舗合計の日別推移グラフと、閲覧数の多い店舗ランキングを表示します。
- ランキングの店舗名をクリックすると、その店舗単体の日別推移に絞り込めます。
