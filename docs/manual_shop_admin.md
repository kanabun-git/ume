...
- 店舗写真(追加方式で最大5枚までアップロード可能。既存の写真は残ったまま追加されます)。編集画面の上部に登録済みの写真がサムネイル表示され、それぞれに「削除」ボタンがあります。不要な写真は個別に削除できます。
...
| ブロック | 内容 |
|---|---|
| 店舗情報 | 住所・電話番号・営業時間・料金・交通費・対応エリア・平均評価を表示(「店舗情報編集」の内容がそのまま反映されます) |
| 求人情報 | コンパニオン募集・スタッフ募集・募集メッセージを表示(「店舗情報編集」の内容がそのまま反映されます。募集していない場合は自動的に非表示になります) |
...
| クイックメニュー | 「トップ」「写メ日記」「出勤情報」「女の子」「動画」「クーポン」「口コミ」「求人情報」など、実際にページに表示されている項目へのジャンプメニューを自動作成(存在しない項目は表示されません。「タイトル帯・枠を非表示にする」をオンにすると見出しの色帯のない細いメニューバーになります) |
| 在籍キャスト | 在籍中のキャスト一覧を表示 |
| プレゼント企画 | 応募受付中のプレゼント企画を表示(受付中の企画が無い場合は自動的に非表示になります) |
| クーポン一覧 | 「クーポン管理」で登録した現在有効なクーポンを、通常料金/割引後料金の表付きで表示(有効なクーポンが無い場合は自動的に非表示になります。ブロックの種類「クーポン」とは別の機能です) |

各ブロックは「▲上に移動」「▼下に移動」で並び替えます。役割の異なる2つの表示切り替えがあり、それぞれ一覧画面の「表示」列・「タイトル帯・枠」列のボタンをクリックするだけで切り替えられます(編集画面を開かなくても操作できます)。混同しやすいので、違いを確認してください。

| 列(ボタン) | 効果 |
|---|---|
| 表示 | オフ(非表示中)にすると、内容ごとそのブロックを店舗ページから完全に非表示にします。ブロック自体が無かったことになります。 |
| タイトル帯・枠 | オフ(非表示中)にしても内容(写真・テキストなど)はそのまま表示されます。見出しの色帯と白い枠(背景・境界線・影)だけを取り除き、ページの背景に馴染ませて表示したい場合に使います。 |

同じ2つの切り替えは各ブロックの「編集」画面のチェックボックス(「表示する」「タイトル帯・枠を非表示にする」)からも設定できます。背景色・不透明度・見出し文字なども編集画面で個別に設定できます。

**「店舗情報」「求人情報」「店舗フォトギャラリー」「新人・体験入店」「店長おすすめ」「写メ日記」「週間出勤」「口コミランキング」「クーポン」「在籍キャスト」「プレゼント企画」「クーポン一覧」の各ブロックには、この画面自体に写真・キャストなどをアップロードする欄はありません。** それぞれ「店舗情報編集」の基本情報・求人情報・店舗写真、「在籍キャスト管理」の在籍中/体験入店/店長おすすめフラグ、キャストの写メ日記投稿、「出勤予定一括登録」の出勤予定、口コミ件数、「クーポン管理」の登録内容、「プレゼント企画管理」の登録内容を、そのまま自動的に表示する仕組みです。この画面ではブロックの表示位置・表示/非表示・見た目(背景色など)だけを管理します。編集画面にも同じ案内が表示されます。
...
- **バナー画像**: 企画カードの上部に横長で表示する画像を設定できます(任意)。推奨サイズは**1200×300px前後(横長)**、5MBまで、JPEG・PNG・WEBP形式です。バナー画像を設定しない場合、「バナー画像が未設定の場合の表示」で以下のいずれかを選べます。
  - **表示しない**: 従来通り、説明文だけのシンプルな企画カードになります。
  - **ZEROの標準画像を表示する**: 画像を用意していなくても、目を引くバナー付きの企画カードにできます(運営者が設定した共通画像、未設定の場合はZERO標準のイラスト画像が使われます)。
  - バナー画像・説明文とも、バナーが上・説明文が下の順で表示されます。
...
![店舗会員証管理一覧画面](images/shop_admin/12_shop_memberships_index.png)
...
- **来店チェックインQRコード(女の子ごと)**: 女の子(キャスト)のスタッフポータル左メニュー「来店チェックインQR」に、そのキャスト専用のQRコードが表示されます。お客様がスマートフォンでこのQRコードを読み込む(会員未ログインの場合はログイン/会員登録を挟んで自動的に読み込み直後の画面に戻ります)と、来店1回として自動記録され、担当キャストにもそのキャストの名前が自動で設定されます。**2重にポイントが付与されるのを防ぐため、その会員の直前の記録(店舗内のどのキャストのQRコードでも)から60分以内は次の記録ができません。**
  - スタッフポータルの画面上でこの画面をお客様に見せて読み込んでもらうほか、「名刺PDFをダウンロード」から名刺サイズ(91×55mm)のQRコードカードをPDFで出力し、印刷して店頭やお客様への手渡し用にお使いいただけます。このカードにはキャストの名前が印字されます。
  - 「店舗会員証管理」画面の一番上にある「在籍キャストの来店チェックインQRカードを一括ダウンロード(印刷用PDF)」から、在籍中の全キャスト分のQRコードカードを1つのPDFにまとめて出力することもできます。A4用紙1枚に10枚(2列×5段・91×55mm)ずつ並べており、市販の名刺用紙(エーワン マルチカードなど、A4・10面・91×55mmのもの)にそのまま印刷できます。**こちらのカードには店舗名のみが印字され、キャストの名前は入りません。**
- **店舗会員証管理**: 登録済みの会員一覧から、会員ごとの詳細画面に入れます。

  ![店舗会員証詳細画面](images/shop_admin/12_shop_membership_show.png)

  詳細画面では以下が行えます。
  - **利用履歴の記録・編集**: 来店日・来店時刻・担当キャスト(女の子)・指名区分(本指名/ネット指名/フリー。未設定のままにしておき、あとから決めることもできます)・時間(分)・獲得ポイント・メモ(会員にも表示されます)を入力して来店を記録します。記録するとポイントが加算され、ランクの基準に達していれば特典が自動付与されます。上記のQRチェックインで記録された来店(一覧に「QRチェックイン」バッジが付きます)は担当キャストのみ自動入力されているため、一覧の「編集」から指名区分・時間などを後から入力してください。
  - **ポイント利用**: 会員が貯めたポイントを使った際、利用ポイント数と内容を入力して記録します。残高を超える利用はできません。
  - **ポイントを手動で付与・修正する**: 上記の「ポイント利用」とは別に、ポイント数(マイナスも可)と内容を入力していつでもポイント履歴を追加できます。残高に関わらず記録でき、誕生日ボーナスなどの手動付与や、記録ミスの修正に使えます。ポイント履歴の一覧からは、各行を「編集」で内容の修正、「削除」で取り消しもできます(来店記録時に自動で作られたポイント履歴も編集・削除できます)。
