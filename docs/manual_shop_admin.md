...
各ブロックの編集画面には、役割の異なる2つのチェックボックスがあります。混同しやすいので、違いを確認してください。

| チェックボックス | 効果 |
|---|---|
| 表示する | オフにすると、内容ごとそのブロックを店舗ページから完全に非表示にします。ブロック自体が無かったことになります。 |
| タイトル帯・枠を非表示にする | オンにしても内容(写真・テキストなど)はそのまま表示されます。見出しの色帯と白い枠(背景・境界線・影)だけを取り除き、ページの背景に馴染ませて表示したい場合に使います。 |
