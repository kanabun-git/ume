...
ポータル本体・コーポレートサイトとは独立した一般公開のツールです。古着のタグや洗濯表示の写真をアップロードすると、ブランド候補・推定年代・中古相場の目安と、その根拠をAIが返します。

| URL | 内容 |
|---|---|
| `/vintage` | 判定フォーム(写真は最大8枚、送信前にブラウザ側で縮小。写真が無くてもタグの表記メモだけで判定可) |
| `/vintage/guide` | 年代判定ガイド(共通の手がかりとブランド別のタグ変遷) |

### AIの切り替え

判定に使うAIは `VINTAGE_AI_PROVIDER` で選びます。プロンプトの組み立てと結果の解釈は `VintageBrandIdentifier` に集約されているので、切り替えても判定の中身と画面は変わりません。

| 値 | 使うAI | 必要な環境変数 | 費用 |
|---|---|---|---|
| `gemini`(既定) | Google Gemini | `GEMINI_API_KEY`(モデルは `GEMINI_MODEL`、既定 `gemini-3.5-flash`) | 無料枠の範囲なら無料 |
| `claude` | Anthropic Claude | `ANTHROPIC_API_KEY`(写メ日記のAI下書きと同じ) | 従量課金(1回あたり数円〜十数円) |

**Geminiの無料枠では、送信した写真とメモがGoogleのサービス改善に利用され、人によるレビューの対象になることがあります。**この事実は判定フォームにも明記しています(`VintageBrandIdentifier.privacy_notice`)。有料枠に切り替えるか `claude` にすると学習には利用されません。
...
* アップロードされた写真は**このサーバーには保存しません**。そのリクエストの中でAPIへ渡すだけで、Active Storageにもディスクにも残しません(`app/models/vintage/identification.rb`)。
* 無料枠の回数制限と待ち時間を抑えるため、IP単位で連続判定のクールダウンと時間あたりの上限を設けています(同ファイルの `COOLDOWN` / `WINDOW_LIMIT`)。
